...
void ge_stop_native_music_cue(PPCContext& ctx, uint8_t* base,
                              uint32_t slot) {
  // Fork note: ge_missing_music_tick calls this with the LIVE hook context, and
  // sub_82144A80 tail-calls through the XACT cue vtable -- it clobbers r3/r4/
  // r9-r11/ctr/cr6. That is the same hazard ge_ce_remote_weapon_sfx documents
  // for the sibling sub_82144920 in this module ("the SFX calls clobber
  // volatile regs; restore after"). Snapshot/restore here so every caller is
  // safe regardless of whether it passed a copy or the real context.
...
// Takes the viewport scan's result rather than repeating it: ge_missing_music_tick
// needs both values every frame, and the scan is up to four guest loads.
uint32_t ge_find_active_player(uint8_t* base, uint32_t viewport_player) {
  uint32_t player = viewport_player;
...
void ge_missing_music_tick(PPCContext& ctx, uint8_t* base) {
  ...
  const uint32_t player = ge_find_active_player(base, active_viewport_player);
...
void ge_mouse_camera(uint8_t* base) {
  ...
  // the online jitter.)
  // (Shared with the music-transition tick -- same +0x904 viewport probe, same
  // GE_BONDVIEW_CUR/GE_PLAYER_PTR fallbacks for menus/boot.)
  const uint32_t player = ge_find_active_player(
      base, ge_find_active_viewport_player(base));
  if (!player) return;
...
void ge_inject_keyboard(PPCRegister& /*r11*/) {
  ...
  // mouse-look support is enabled -- deliberately above the ge_mouselook_enable
  // gate below, since Android forces that cvar off (see ge_app.h).
